...
另有超过 20 个可选的字段，如：总脂肪、饱和脂肪、反式脂肪、胆固醇、钠等等。大多数产品只含有少数的可选字段，且字段值为非零值。
...
传统的方式是使用可伸缩构造函数，在这种模式中，只向构造函数提供必需的参数。
即，向第一个构造函数提供一个可选参数，向第二个构造函数提供两个可选参数，以此类推，最后一个构造函数是具有所有可选参数的。
...
简单地说，**可伸缩构造函数模式可以工作，但是，当有很多参数时，编写客户端代码很困难，而且读起来更困难。** 
读者想知道所有参数值的意义，必须仔细清点参数。相同类型的长序列参数会导致不易察觉的的 bug。
如果客户端不小心颠倒了两个这样的参数，编译器不会报错，但是程序会在运行时出错（[Item-51](../8.Methods/8.51-Design-method-signatures-carefully.md)）。
...
另一个的缺点是，**JavaBean 模式不可能使类不可变**（[Item-17](../4.Classes-and-Interfaces/4.17-Minimize-mutability.md)），并且需要程序员额外的努力来确保线程安全。
...
然后，客户端在 builder对象上调用像 setter 这样的方法来设置每个感兴趣的可选参数。
最后，客户端调用一个无参数的构建方法来生成对象，这通常是不可变的。 
构建一个类的 Builder通常是这个类的 static 成员类（[Item-24](../4.Classes-and-Interfaces/4.24-Favor-static-member-classes.md)）。下面是它在实际应用中的样子：
...
为了确保这些不变量不受攻击，在从构建器复制参数之后检查对象字段（[Item-50](../8.Methods/8.50-Make-defensive-copies-when-needed.md)）。
如果检查失败，抛出一个 IllegalArgumentException（[Item-72](../10.Exceptions/10.72-Favor-the-use-of-standard-exceptions.md)），
它的详细消息指示哪些参数无效（[Item-75](../10.Exceptions/10.75-Include-failure-capture-information-in-detail-messages.md)）。
...
**Builder模式非常适合于类层次结构**。使用构建器的并行层次结构，
...
请注意，`Pizza.Builder` 是具有递归类型参数的泛型类型（[Item-30](../5.Generics/5.30-Favor-generic-methods.md)）。
它与 abstract self 方法一起，让方法链式调用在子类中正常工作，而不需要强制转换。
对于 Java 缺少自类型这一事实，这种变通方法称为 _模拟自类型_ 习惯用法。
这里有两个具体的Pizza子类，一个是标准的纽约风格的比萨，另一个是 calzone。
前者需要一个size参数，而后者让你指定酱料应该是内部还是外部：
...
但是，如果你以构造函数或静态工厂开始，直至类扩展到参数数量无法控制的程度时，才切换到构建器，那么，过时的构造函数或静态工厂将很难处理。
因此，最好一开始就从构建器开始。
...
总之，在设计构造函数或静态工厂的类时，Builder模式是一个很好的选择，特别是当许多参数是可选的或具有相同类型时。
